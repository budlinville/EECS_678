...
#define DEADLOCK 0            //can change to 0
...
int check_for_deadlock()
{
  ...
  for (i = 0; i < NUM_PHILS; i++) {
     ...
     */
     if ((statf = fopen(filename, "r")) < 0) {
       printf("Statfile %s could not be opened.", filename);
       exit(0);
     }
     ...
     */
     if(user_time[i] != new_user_time || new_sys_time != sys_time[i]) {
      deadlock = 0;
