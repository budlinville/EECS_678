...
void check_jobs_bg_status() {
  ...
  for (size_t i = 0; i < jobsLength; i++){
	  ...
	  if (is_empty_PIDDeque(&thisJob.pid_list)) {
		  //all processes complete => job complete
...
void create_process(CommandHolder holder, ExecEnv* exec_env) {
  ...
  if (pid == 0) {
	...
	if (r_in) {
	  pIn = open(holder.redirect_in, O_RDONLY);
	...
	if (r_out) {
	  if (r_app) {
		  pOut = open(holder.redirect_out, O_WRONLY | O_APPEND | O_CREAT, 0664);
	  } else {
		  pOut = open(holder.redirect_out, O_WRONLY | O_TRUNC | O_CREAT, 0664);
	  }
