...
static void *dp_thread(void *arg)
{
  ...
  while (!Stop) {
    ...
    while (!(right_chop_available && left_chop_available)) {
      pthread_cond_wait(&(me->can_eat), &waiter);
    }

    //lets define left chopstick as chopstick at index equal to a philosopher's id
    //right chopstick is equal to (philospher's id) + 1
    //mod NUM_PHILOSPHERS
    int leftChopIndex = me->id;
    int rightChopIndex = (me->id - 1) % NUM_PHILS;
    available_chopsticks[leftChopIndex] = 0;
    available_chopsticks[rightChopIndex] = 0;
    ...
    available_chopsticks[leftChopIndex] =1;
    available_chopsticks[rightChopIndex] = 1;
